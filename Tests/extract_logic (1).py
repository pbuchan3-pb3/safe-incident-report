...
# 6. Draft helpers (DRAFT_KEY, loadDraft, formatDraftAge — pure logic, no DOM)
s = content.find('var DRAFT_KEY =') or content.find('const DRAFT_KEY =')
if s == -1: s = content.find('DRAFT_KEY =')
e_load = content.find('\nfunction loadDraft(){')
e_load_end = content.find('\n}', e_load) + 2
e_fmt = content.find('function formatDraftAge(savedAt){')
e_fmt_end = content.find('\n}', e_fmt) + 2
if s != -1 and e_load != -1 and e_fmt != -1:
    blocks['draftHelpers'] = content[s:e_load_end].strip() + '\n\n' + content[e_fmt:e_fmt_end].strip()
else:
    print('WARNING: draftHelpers block not found')
