...
// ════════════════════════════════════════════════════════════
// Tests the pure logic of speakRewritePreview — what text it
// builds, how it chunks, and that displayed === spoken text.
// (Actual SpeechSynthesis API is browser-only; we test the logic.)

var CHUNK_LIMIT = 200;

function buildSpeakPhrase(cleanedText, contextLabel){
  var label = contextLabel || "Here's how it will read on the report";
  return (label + ': ' + cleanedText).replace(/<[^>]+>/g,'').replace(/&nbsp;/g,' ').trim();
}

function chunkForSpeech(plain){
  if(plain.length <= CHUNK_LIMIT) return [plain];
  var sentences = plain.match(/[^.!?]+[.!?]+|[^.!?]+$/g) || [plain];
  var chunks = [];
  var current = '';
  for(var s of sentences){
    if((current + s).length > CHUNK_LIMIT && current.length > 0){
...
{ var cleaned = 'The female subject was always the first to volunteer to move to different sections to benefit the company, doing so without complaints and with a pleasant smile.';
  var displayed = `Here's how it will read on the report: "${cleaned}"`;
  var spoken = buildSpeakPhrase(cleaned, "Here's how it will read on the report");
  // The spoken text is the label + cleaned; the displayed text wraps cleaned in quotes.
  // The KEY requirement: the spoken cleaned text MUST be identical to the displayed cleaned text.
  check('Spoken cleaned text equals displayed cleaned text', spoken.includes(cleaned));
  check('Spoken phrase starts with context label', spoken.startsWith("Here's how it will read on the report")); }

// ── Short text: no chunking needed ───────────────────────────
{ var short = 'The female subject was wearing a black shirt.';
  var chunks = chunkForSpeech(buildSpeakPhrase(short));
  check('Short text: exactly 1 chunk', chunks.length === 1);
  check('Short text: chunk contains the full text', chunks[0].includes(short)); }

// ── Long text: chunked at sentence boundaries ─────────────────
{ var long = "Here's how it will read on the report: The employee consistently demonstrated exceptional leadership and initiative throughout the event. She proactively assisted guests with accessibility needs, resolved seating disputes professionally, and maintained a calm demeanor under pressure. Her contributions were recognized by multiple guests and fellow staff members.";
  var chunks = chunkForSpeech(long);
  check('Long text: multiple chunks produced', chunks.length > 1);
  check('Long text: each chunk under limit', chunks.every(c => c.length <= CHUNK_LIMIT));
  // Reassembled chunks should contain all the key content
  var reassembled = chunks.join(' ');
  check('Long text: no content lost in chunking', reassembled.includes('accessibility needs') && reassembled.includes('fellow staff members')); }

// ── Chunking does not truncate mid-word ───────────────────────
{ var text = 'A'.repeat(150) + '. ' + 'B'.repeat(150) + '. ' + 'C'.repeat(50) + '.';
  var chunks = chunkForSpeech(text);
  check('Chunking: no chunk exceeds limit', chunks.every(c => c.length <= CHUNK_LIMIT));
  check('Chunking: more than 1 chunk for long input', chunks.length > 1); }

// ── HTML stripped from spoken text ───────────────────────────
{ var htmlCleaned = '<em>"The female subject was wearing a black-and-white shirt."</em>';
  var plain = htmlCleaned.replace(/<[^>]+>/g,'').replace(/&nbsp;/g,' ').trim();
  check('HTML stripped: no tags in spoken text', !plain.includes('<'));
  check('HTML stripped: content preserved', plain.includes('black-and-white shirt')); }

// ── Context label variants ────────────────────────────────────
{ var text2 = 'Demonstrated excellent teamwork.';
  var v1 = buildSpeakPhrase(text2, "Here's how it will read");
  var v2 = buildSpeakPhrase(text2, "Here's how it will read on the report");
  var vDefault = buildSpeakPhrase(text2, null);
  check('Label variant 1: correct prefix', v1.startsWith("Here's how it will read:"));
  check('Label variant 2: correct prefix', v2.startsWith("Here's how it will read on the report:"));
  check('Label null: defaults to report label', vDefault.startsWith("Here's how it will read on the report:")); }

// ── All rewrite sites have suppressSpeak=true ─────────────────
// We verify this by checking the app source file
{ var appSrc = require('fs').readFileSync(require('path').join(__dirname, '..', 'safe_incident_form_24.html'), 'utf8');

  // Every "Here's how it will read" addBubble call must have the 3rd arg
  var heresBubbles = appSrc.match(/addBubble\(`Here's how it will read[^`]*`,'ai'[^)]*\)/g) || [];
  var suppressed = heresBubbles.filter(b => b.includes('true'));
  check("All 'Here's how it will read' bubbles have suppressSpeak=true",
    heresBubbles.length > 0 && heresBubbles.length === suppressed.length,
    `${suppressed.length}/${heresBubbles.length} suppressed`);

  // speakRewritePreview is defined
  check('speakRewritePreview is defined in app', appSrc.includes('function speakRewritePreview(cleanedText'));

  // No more direct speak("Writing that up...") calls
  check('No redundant speak("Writing that up...") calls remain',
    !appSrc.includes('speak("Writing that up in professional language")') &&
    !appSrc.includes("speak('Writing that up in professional language')")); }

// ── speakRewritePreview call sites ────────────────────────────
{ var appSrc2 = require('fs').readFileSync(require('path').join(__dirname, '..', 'safe_incident_form_24.html'), 'utf8');
  var callSites = (appSrc2.match(/speakRewritePreview\(/g) || []).length;
  // 1 definition + should be at least 9 call sites (one per rewrite workflow)
  check('speakRewritePreview called at 9+ rewrite sites', callSites >= 10,
    `found ${callSites} total references`); }

// ── addBubble suppressSpeak parameter is backward compatible ──
{ // addBubble(text, role) — no 3rd arg — still speaks normally
  var normalBubbleSuppressed = (undefined === true); // false — should not suppress
  check('addBubble without suppressSpeak: auto-speak still fires', !normalBubbleSuppressed);

  // addBubble(text, role, true) — suppresses
  var suppressed2 = (true === true);
  check('addBubble with suppressSpeak=true: auto-speak suppressed', suppressed2); }
