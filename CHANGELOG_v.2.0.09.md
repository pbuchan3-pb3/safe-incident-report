...
## [2.0.9] — Recognition Intelligence and Interview Refinement (staged; field verification pending)
NOTE: v2.0.8 remains production while Android field verification is under review.
v2.0.9 is a staged candidate and is NOT production-approved.
- Category-driven recognition branching via RECOGNITION_CATEGORY_CONFIG + generic
  helpers (recognitionUsesGuestCount/Benefit/Leadership/Safety, buildRecognitionInterview).
  New categories are added by configuration, not new interview code. Guest count is
  asked ONLY for Guest Service; Teamwork asks who benefited; Leadership/Safety/
  Initiative/Professionalism each ask one relevant question; Other asks none.
- Structured observed behaviors: 16-option multi-select (OBSERVED_BEHAVIOR_OPTIONS)
  replacing free-form; stored as recognitionObservedBehaviors[] and joined into
  recognitionObservations for backward compatibility. "Other" collects text.
- Dynamic confirmed-name wording via existing empRef()/isConfirmedPersonName;
  witness prompt now "What did <Name> tell you?"; never exposes Unknown/Refused.
- Optional recognition recommendation (Recognition Only … Annual Recognition /
  Other / Skip); omitted from exports when empty.
- Empty-section suppression for optional recognition sections (Additional
  Observations, Additional Comments, Recommendation, beneficiary/count) across
  text, PDF, and Word — no "None"/"N/A"-only sections.
- Beneficiary + guest count rendered in text/PDF/Word/Sheets only when meaningful.
- Narrative enhancement via editorial CONTEXT only (profiles untouched): encourages
  what happened / why exceptional / how it helped operations / positive impact —
  factual, no invention.
- All 20 locked systems byte-identical (recorder, transcription, Worker, assignment
  selector, name parser, entity renderer, native sharing, Field Mode, operational
  context). generatePDF/generateWord changed only in recognition blocks. Worker unchanged.
